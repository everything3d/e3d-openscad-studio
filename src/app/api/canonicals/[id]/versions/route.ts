...
export async function POST(req: Request, { params }: Params) {
  ...
  if (!parsed.success) return NextResponse.json({ error: 'Invalid starter version' }, { status: 400 })
  ...
  if (!canonical) return NextResponse.json({ error: 'Starter or workspace not found' }, { status: 404 })
